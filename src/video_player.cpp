...
bool VideoPlayer::open(const std::string& filepath) {
    ...
    // 检查是否有音频流
    has_audio_ = (stream_info_.audio_stream_index >= 0);
    if (has_audio_) {
        // 打开音频解码器
        if (!audio_decoder_->open(filepath)) {
            std::cerr << "Warning: Failed to open audio decoder, will play video only" << std::endl;
            has_audio_ = false;
        }
    }
...
bool VideoPlayer::initializeAudio() {
    ...
    MKVStreamReader::StreamInfo audio_stream_info = stream_reader->getStreamInfo();
    if (audio_stream_info.audio_stream_index < 0) {
        std::cerr << "No audio stream found in audio decoder" << std::endl;
        return false;
    }
    
    // 初始化 WASAPI 音频播放器
    UINT32 sample_rate = audio_stream_info.audio_sample_rate;
    UINT16 channels = static_cast<UINT16>(audio_stream_info.audio_channels);
...
bool VideoPlayer::isAudioReady() const {
    return has_audio_ && is_audio_playing_;
}


bool VideoPlayer::getAudioBufferStatus(UINT32& total_frames, UINT32& padding_frames, UINT32& available_frames) const {
    if (!has_audio_ || !is_audio_initialized_) {
        return false;
    }
...
void VideoPlayer::handleAudioFrames(double current_time) {
    if (!has_audio_ || !is_audio_initialized_) {
        return;
    }
...
void VideoPlayer::preloadAudioFrames() {
    if (!has_audio_ || !audio_decoder_) {
        return;
    }
...
double VideoPlayer::convertAudioPtsToSeconds(AVFrame* frame) const {
    if (!frame || frame->pts == AV_NOPTS_VALUE) {
        return 0.0;
    }
...
bool VideoPlayer::initializeWASAPIAudioPlayer(UINT32 sample_rate, UINT16 channels, UINT16 bits_per_sample) {
...
bool VideoPlayer::writeAudioData(AVFrame* frame) {
...
bool VideoPlayer::startAudioPlayer() {
...
void VideoPlayer::stopAudioPlayer() {
    if (!is_audio_initialized_ || !is_audio_playing_) {
        return;
    }
...
void VideoPlayer::closeAudioPlayer() {
...
bool VideoPlayer::initializeAudioDevice() {
    // 创建设备枚举器
...
bool VideoPlayer::configureAudioClient() {
    // 获取混音格式
...
void VideoPlayer::cleanupAudioResources() {
...
bool VideoPlayer::convertFloatToPcm(AVFrame* frame, BYTE* buffer, UINT32 frames_to_write) {
    if (!frame || !buffer) {
        return false;
    }
