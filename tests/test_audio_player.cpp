...
TEST_CASE("AudioPlayer error handling", "[audio_player]") {
    AudioPlayer player;
    
    SECTION("Test operations on unopened player") {
        // 未打开文件时的操作应该失败
        REQUIRE_FALSE(player.initialize());
        REQUIRE_FALSE(player.isReady());
        ...
        // 获取音频解码器应该返回空指针
        REQUIRE(player.getAudioDecoder() == nullptr);
        
        // 获取音频缓冲区状态应该失败
        UINT32 total_frames, padding_frames, available_frames;
        REQUIRE_FALSE(player.getAudioBufferStatus(total_frames, padding_frames, available_frames));
    }
    
    SECTION("Test operations on opened but uninitialized player") {
        std::ifstream file(AUDIO_TEST_FILE);
        if (!file.good()) {
            SKIP("Test file " << AUDIO_TEST_FILE << " not found");
        }
