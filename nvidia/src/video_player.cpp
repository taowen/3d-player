...
VideoPlayer::VideoPlayer() 
    : d3d_stereo_decoder_(std::make_unique<D3dStereoVideoDecoder>()) {
}
...
bool VideoPlayer::open(const std::string& filepath) {
    ...
    // 打开立体视频解码器
    if (!d3d_stereo_decoder_->open(filepath)) {
        std::cerr << "Failed to open stereo decoder for: " << filepath << std::endl;
        return false;
    }
    
    // D3dStereoVideoDecoder 不暴露内部实现细节，无需获取底层组件
    // 时间戳信息从 DecodedStereoFrameD3D 的 AVFrame 中计算获取
...
void VideoPlayer::onTimer(double current_time) {
    ...
    while (!frame_buffer_.empty()) {
        ...
        if (front_frame.stereo_frame.input_frame && front_frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame && front_frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame->pts != AV_NOPTS_VALUE) {
            AVRational time_base = d3d_stereo_decoder_->getVideoTimeBase();
            frame_time = front_frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame->pts * av_q2d(time_base);
        }
        ...
        if (frame_time <= current_time) {
            ...
            // 设置新的当前帧
            current_frame_texture_ = front_frame.d3d_texture;
            current_frame_time_ = frame_time;
            
            // 释放AVFrame内存
            if (front_frame.stereo_frame.input_frame && front_frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame) {
                av_frame_free(&front_frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame);
            }
            frame_buffer_.pop();
...
void VideoPlayer::close() {
    ...
    // 清空帧缓冲
    while (!frame_buffer_.empty()) {
        DecodedStereoFrameD3D& frame = frame_buffer_.front();
        if (frame.stereo_frame.input_frame && frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame) {
            av_frame_free(&frame.stereo_frame.input_frame->rgb_frame.hw_frame.frame);
        }
        frame_buffer_.pop();
    }
    ...
    // 关闭解码器
    if (d3d_stereo_decoder_) {
        d3d_stereo_decoder_->close();
    }
}


bool VideoPlayer::isOpen() const {
    return d3d_stereo_decoder_ && d3d_stereo_decoder_->isOpen();
}
...
bool VideoPlayer::isEOF() const {
    return d3d_stereo_decoder_ && d3d_stereo_decoder_->isEOF();
}


D3dStereoVideoDecoder* VideoPlayer::getStereoDecoder() const {
    return d3d_stereo_decoder_.get();
}
...
ID3D11Device* VideoPlayer::getD3D11Device() const {
    ...
    return d3d_stereo_decoder_->getD3D11Device();
}
...
ID3D11DeviceContext* VideoPlayer::getD3D11DeviceContext() const {
    ...
    // D3dStereoVideoDecoder 不暴露 DeviceContext，通过设备获取
    ID3D11Device* device = d3d_stereo_decoder_->getD3D11Device();
    if (!device) {
        return nullptr;
    }
...
bool VideoPlayer::preloadNextFrame() {
    ...
    if (!d3d_stereo_decoder_->readNextFrame(frame)) {
...
int VideoPlayer::getWidth() const {
    ...
    return d3d_stereo_decoder_->getWidth();
}
...
int VideoPlayer::getHeight() const {
    ...
    return d3d_stereo_decoder_->getHeight();
} 